  ...
  calculate_denominator<-function(knowns, allele_combos, full_table, num_cont, names){
    ...
    write.csv(full_table, "denominator.csv")
  ...
  generate_full_dropin_table<-function(quant, rep_1, rep_2, rep_3, full_allele_table, number_contributors, deducible_nondeducible){
    full_table<-rep(NA, nrow(full_allele_table))
    if(!("INC"%in%rep_1) & length(rep_1) > 0){
      
      full_rep1_table<-apply_dropin(quant, full_allele_table, rep_1, number_contributors, deducible_nondeducible)
      full_table<-cbind(full_table, full_rep1_table[,ncol(full_rep1_table)])
      #full_table<-full_rep1_table
    }
    if(!("INC"%in%rep_2) & length(rep_2) > 0){
      full_rep2_table<-apply_dropin(quant, full_allele_table, rep_2, number_contributors, deducible_nondeducible)
      full_table<-cbind(full_table, full_rep2_table[,ncol(full_rep2_table)])
    }
    if(!("INC"%in%rep_3) & length(rep_3) > 0){
      full_rep3_table<-apply_dropin(quant, full_allele_table, rep_3, number_contributors, deducible_nondeducible)
  ...
  calculate_all_LRfunction<-function(locus_alleles,rep_1_alleles, rep_2_alleles, rep_3_alleles, locus, number_contributors, deducible_nondeducible, quantity, n_known1, n_known2, n_known3, 
    ...
    full_dropin_table<-generate_full_dropin_table(quantity, rep_1_alleles[,"V2"], rep_2_alleles[,"V2"], rep_3_alleles[,"V2"], full_allele_table, number_contributors, deducible_nondeducible)
  ...
  server<- function(input, output){
    observeEvent(input$submitButton, {
     ...
     #calculate_all_LRfunction(CSF_alleles_freq,Rep_1_CSF, Rep_2_CSF, Rep_3_CSF, "CSF", number_contributors, deducible_nondeducible, quantity, n_CSF1, n_CSF2,n_CSF3, d_CSF1, d_CSF2, d_CSF3, "V3")
     calculate_all_LRfunction(D8_alleles_freq,Rep_1_D8, Rep_2_D8, Rep_3_D8, "D8", number_contributors, deducible_nondeducible, quantity, n_D81, n_D82,n_D83, d_D81, d_D82, d_D83, "V3")
